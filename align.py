...
def main_worker(dist):
    ...
    # data prepare
    root = os.path.abspath(os.path.join(os.path.expanduser('~'), 'datasets', args.dataset))
    ...
    memory_data = CIFAR10(root=root, train=True, transform=test_transform, download=False)
    memory_loader = DataLoader(memory_data, batch_size=args.batch_size, shuffle=False, num_workers=4, pin_memory=True)
    
    test_data = CIFAR10(root=root, train=False, transform=test_transform, download=False)
