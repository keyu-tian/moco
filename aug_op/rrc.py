...
C_in = -1
@numba.jit(nopython=True, nogil=True, fastmath=True)    # , parallel=True
def filter_param_pair(AB_range_lb: float, AB_range_ub: float, C_range_lb: float, C_range_ub: float, HW: int, target_num: int, verbose: bool):
    params = []
    IoUs = []
    ...
    while cnt < target_num:
        ...
        if AB_range_lb < AB < AB_range_ub and C_range_lb < AC < C_range_ub and C_range_lb < BC < C_range_ub:
...
if __name__ == '__main__':
    print(get_params(32, 'AB_0.0_0.3', 100000, True).shape)
